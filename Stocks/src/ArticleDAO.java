...
public class ArticleDAO {
	...
	public int ajouter(Article article) {
		...
		try {
			...
			ps.setString(2, article.getprenom());
			ps.setString(3, article.getdate_nais());
			ps.setString(4, article.getfonction());
	...
	public Article getArticle(int reference) {
		...
		try {
			...
			if (rs.next())
				retour = new Article(rs.getString("art_reference"),
	...
	public List<Article> getListeArticles() {
		...
		try {
			...
			while (rs.next())
				retour.add(new Article(rs.getString("art_reference"), rs
