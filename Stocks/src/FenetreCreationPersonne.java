...
/**
 * Classe FenetreCreationPersonne
 * Définit et ouvre une fenetre qui :
 * 
 *    - Permet l'insertion d'une nouvelle personne dans la table personne via
 * la saisie des valeurs de nom, prénom, date de naissance et fonction à l'école
...
public class FenetreCreationPersonne extends JFrame implements ActionListener {
	/**
	 * numero de version pour classe serialisable Permet d'eviter le warning
	 * "The serializable class FenetreCreationPersonne does not declare a static final serialVersionUID field of type long"
	 */
	private static final long serialVersionUID = 1L;

	/**
	 * conteneur : il accueille les differents composants graphiques de
	 * FenetreCreationPersonne
	 */
	...
	public void actionPerformed(ActionEvent ae) {
		...
		try {
			if (ae.getSource() == boutonCreer) {
				...
				retour = unePersonneDAO.ajouter(a);
				...
				// on demande à la classe PersonneDAO d'ajouter le message
				// dans la base de données
				List<Personne> liste = unePersonneDAO.getListePersonnes();
